...
def careers(request):
    # Fetch all current careers
    current_careers = Careers.objects.all()

    # Pass the careers to the template
    context = {'current_careers': current_careers}
    return render(request, 'yourapp/careers_list.html', context)
